...
onMounted(async () => {
  ...
  // CRÍTICO: Verificar y cargar estado específico del usuario
  await cargarEstadoUsuarioEspecifico();
  
  // IMPORTANTE: Verificar inmediatamente después de cargar
  setTimeout(async () => {
    await verificarYMantenerPersistencia();
    
    // Si está bloqueado, asegurar que se mantenga
    if (estadoBloqueo.value === 'bloqueado') {
      console.log(`🔒 CONFIRMADO: Usuario ${nombre.value} está BLOQUEADO al iniciar`);
      await forzarGuardadoCompleto();
    }
  }, 1000);
  ...
  if (import.meta.env.DEV) {
    ...
    console.log('🔧 Sistema de bloqueo persistente por usuario específico activado');
    
    // En desarrollo, mostrar estado después de 2 segundos
    setTimeout(() => {
